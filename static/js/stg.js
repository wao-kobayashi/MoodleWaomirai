...
//moodleで友だち追加
const UrlLiffMoodle =
  "https://liff.line.me/2006716288-lL7QzGA3?loycus_urlc=y7vy";
const ImgLiffMoodle =
  "https://go.waomirai.com/l/1026513/2025-04-04/hk1f8/1026513/1743763011hBkCW5F9/ImgLiffMoodlefix.png";

// ==============================
// googleCalender系
// ==============================

const iframeCalenderPhilosophy =
  "https://calendar.google.com/calendar/embed?src=c_57f70f2fb986aabbb85c2a71ed169e2624e902265abb6dffc1d993f2d781dd4b%40group.calendar.google.com&ctz=Asia%2FTokyo"; //哲学
const iframeCalenderScience =
  "https://calendar.google.com/calendar/embed?src=c_9d34850398ee79fea558cb874c3bebe48860ce3d5fcff5c80f91b203974af452%40group.calendar.google.com&ctz=Asia%2FTokyo"; //科学
const iframeCalenderEconomy =
  "https://calendar.google.com/calendar/embed?src=c_9ee064fdb148232860ebd82900e5222d5060f6702f608257b9d9625d6bcd3a1c%40group.calendar.google.com&ctz=Asia%2FTokyo"; //経済
const iframeCalenderEnglish =
  "https://calendar.google.com/calendar/embed?src=c_379c34d3c8e6716b3458dd339f4531bd8ce07f17c4f97d5fec4367888a692290%40group.calendar.google.com&ctz=Asia%2FTokyo"; //グローバル英語

// ==============================
// 授業のメモシート
// ==============================

const memosheetPhilosophy =
  "https://waomirai.com/lp/assets/moodle/memosheet_philosphy.pdf"; //哲学
const memosheetScience =
  "https://waomirai.com/lp/assets/moodle/memosheet_science.pdf"; //科学
const memosheetEconomy =
  "https://waomirai.com/lp/assets/moodle/memosheet_economy.pdf"; //経済
...
const hasBoughtAbroadSubject = checkGroup(
  (subject) => subject.key === "abroad"
);
...
function createModal(options = {}) {
      ...
      .map(
        (button) =>
          `<a href="${button.url || "#"}" class="c-modal-wrap-button ${
            ...
            button.blank ? ' target="_blank" rel="noopener noreferrer"' : ""
        ...
        ${
          options.image
            ? `<div class="c-modal-wrap-image">
                 <img src="${options.image}" alt="Modal Image" class="${
                options.imageClass || ""
              }" />
               </div>` // モーダルに表示する画像
            : ""
  ...
  // 閉じるボタンをクリックした場合の処理
  $(".c-modal-wrap-close,.c-modal-wrap-closetxt,.c-modal-wrap-close-tag").on(
    "click",
    function () {
...
if (bodyId === "page-my-index") {
  ...
  function displayMainSubjectStatus() {
    ...
    const subjectMainNames = subjects
      ...
      .filter((subject) => {
        ...
        console.log(
          `科目チェック: ${subject.name}, サブ科目あり: ${hasSubjectChild}`
        );
      ...
      .map((subject) =>
        renderDisplaySubject(subject, getSubjectIcon(subject), true)
      )
      .join("");
  ...
  function displayChildSubjectStatus() {
    ...
    const subjectChildNames = subjects
      ...
      .map((subject) =>
        renderDisplaySubject(subject, getSubjectIcon(subject), false)
      )
      .join("");
  ...
  wrappedContent
    .find(".dashboard-left-block-guide")
    .addClass("dashboard-left-block-guide-scroll");
  ...
  function updateClassSchedule() {
    ...
    if (checkAbroadUser()) {
      // 条件を満たす場合、ラベルを表示
      // 定数としてラベルのHTMLを定義
      const abroadUserJstLabel =
        '<div class="p-abroad-user-jst-label">授業時間は日本時間(JST)での表示です</div>';
      // PC用のラベルを挿入
      $(".block_calendar_month").append(abroadUserJstLabel);
      // スマートフォン用のラベルを挿入
      $(".dashboard-main-navi + .dashboard-main-class .card-title").after(
        abroadUserJstLabel
      );
    }
    ...
    if (
      todayDay >= DayChangeCourseBannerStart &&
      todayDay <= DayChangeCourseDeadLine &&
      !$.cookie(monthlyChangeCourseCookie) &&
      hasBoughtMainSubject
    ) {
      // 条件を満たす場合、モーダルを表示
      createModal({
        title:
          "【ご案内】<br />受講レベル及び科目の変更<br />ご解約は当月" +
          DayChangeCourseDeadLine +
          "日までに<br />お手続きをお願いします。<br /><br />",
        buttons: [
          // OKボタンを追加
      ...
      $.cookie(monthlyChangeCourseCookie, "true", {
        expires: DayChangeCourseDeadLine - DayChangeCourseBannerStart + 1,
      });
    }
    ...
    if (checkAbroadUser() && !$.cookie(abroadUserCookie)) {
      ...
      createModal({
        title:
          "授業時間は「日本時間(JST)」に<br />基づいて表示されます。<br /><br />",
        buttons: [
          // OKボタンを追加
          { text: "確認しました", class: "btn-primary c-modal-wrap-close-tag" },
        ],
      ...
      if ($dayContent.length > 0) {
        ...
        $events.each(function () {
          ...
          if (courseName.includes("経済")) {
            ...
            $eventLink.attr(
              "style",
              "background: #28afe7 !important; border-left: #008EC9 2px solid !important;"
            );
    ...
    setTimeout(() => {
      ...
      if (!hasBoughtAdminSubject) {
        ...
        let previousMonth = new Date(
          parseInt($(".arrow_link.previous").attr("data-year"), 10),
          parseInt($(".arrow_link.previous").attr("data-month"), 10) - 1
        );
...
if (bodyId === "page-login-signup" || bodyId === "page-login-forgot_password") {
  ...
  const placeholders = {
    ...
    id_profile_field_wao_schoolname:
      "例）能開⚪︎⚪︎校、Axis⚪︎⚪︎校、オンライン家庭教師", // ワオ未来塾校名のプレースホルダー
  };
...
if (bodyId === "page-enrol-index") {
  ...
  const isHtmlCopy =
    searchParams.has("htmlCopy") || searchParams.get("params") === "htmlCopy";
  ...
  if (DayDisabledFee == DayOfMonth) {
    // 追従のタグを追加
    $("#page-enrol-index").append(
      '<div class="disabled-fee-fixed"><span class="icon-disabled-fee-fixed">&#x26a0;&#xfe0f;</span>毎月' +
        DayDisabledFee +
        '日はシステムメンテナンスのため、受講登録手続きができません。<br class="br-disabled-fee-fixed">お手数ですが、翌日以降に手続きをお願いします。</div>'
    );
  ...
  if (
    ...
    if ($buttonElement.length) {
      ...
      $(document).on("click", ".view-details-link", function (event) {
        ...
        const twosubjectpackId = subjects.find(
          (subject) => subject.key === "twosubjectpack"
        ).id;
        const threesubjectpackId = subjects.find(
          (subject) => subject.key === "threesubjectpack"
        ).id;
        ...
        createModal({
          ...
          buttons: [
            {
              text: "2教科を受講：11,000円(税)/月",
              url: `https://lms.waomirai.com/enrol/index.php?id=${twosubjectpackId}`,
              class: "btn-primary",
            }, // 2教科セットのリンク
            {
              text: "3教科を受講：15,400円(税)/月",
              url: `https://lms.waomirai.com/enrol/index.php?id=${threesubjectpackId}`,
              class: "btn-primary",
            }, // 3教科セットのリンク
          ],
        });
  ...
  $(".enrol_fee_payment_region button").on("click", function (event) {
    ...
    if (DayDisabledFee == DayOfMonth) {
      createModal({
        title:
          "️️毎月" +
          DayDisabledFee +
          "日はシステムメンテナンスのため<br />受講登録手続きができません。<br />お手数ですが、翌日以降に<br />手続きをお願いします。<br /><br />",
        buttons: [
          // OKボタンを追加
          { text: "確認しました", class: "btn-primary c-modal-wrap-close-tag" },
        ],
    ...
    if (["twosubjectpack", "threesubjectpack"].includes(subjectCategory)) {
      ...
      if (checkBoughtMainSubject(["philosophy", "science", "economy"])) {
        ...
        subjectCategory === "twosubjectpack" &&
        checkBoughtMainSubject(["threesubjectpack"])
      ) {
        ...
        subjectCategory === "threesubjectpack" &&
        checkBoughtMainSubject(["twosubjectpack"])
      ) {
        event.stopImmediatePropagation();
        $("body").append(
          createModal({
...
// ==============================
if (
  bodyId === "page-mod-questionnaire-view" ||
  bodyId === "page-mod-questionnaire-complete" ||
  bodyId === "page-mod-questionnaire-report" ||
  bodyId === "page-mod-questionnaire-myreport"
) {
  ...
  const spCourseLessonDate = $("<div>", {
    // 新しいdivにIDを設定（スマホ版のページヘッダー）
    class: "c-pc-hidden", // デスクトップ版では非表示にするためのクラス（PC版では隠す）
  ...
  const textQuestionnaireNotAnswered =
    "<p>授業の視聴が終わったら課題を提出しましょう<br />毎月の課題を全部提出すると、<span class='open-modal-badge'>スペシャルなバッジ</span>がゲットできます！<br /><br />今月のバッジ、ゲットできるかな？さあ、課題を提出してみましょう！</p>";
  ...
  $(".qn-answer textarea").attr(
    "placeholder",
    textQuestionnaireTextareaPlaceholder
  );
  ...
  if ($(".surveyTitle").text().includes("ありがとう")) {
    $(".mod_questionnaire_completepage h3").after(
      ButtonQuestionnaireBacktoCalender
    );
  }
}
...
$(".open-modal-badge").click(function () {
  ...
  createModal({
    image:
      "https://go.waomirai.com/l/1026513/2025-04-01/hjs1g/1026513/17434933816M8hgzmi/modal_badge_sample.png",
...
if (
  (bodyId === "page-course-view-flexsections" ||
    bodyId === "page-course-view-topics") &&
  ...
  $('.course-section-header [data-for="sectiontoggler"]').each(function () {
    ...
    // まとめシートは上記３教科のため
    if (
      ["philosophy", "science", "economy"].includes(
        currentViewCourseData?.key
      ) &&
      ariaLabel &&
      ariaLabel.includes("年") &&
      ariaLabel.includes("月")
    ) {
      // modtype_resource 内の activity-icon の href を取得して削除
      var hrefList = $courseSection
        .find(".modtype_resource")
        .map(function () {
  ...
  if (currentViewCourseData?.type === "main") {
    ...
    const currentMainSubject = subjects.find(
      (subject) =>
        subject.key === currentMainSubjectKey && subject.type === "main"
    );
    ...
    if (
      currentMainSubjectKey === "twosubjectpack" ||
      currentMainSubjectKey === "threesubjectpack"
    ) {
...
function showCampaignModal() {
  createModal({
    ...
    buttons: [
      {
        text: "OKです",
        class: "btn-primary c-modal-level-setting c-modal-wrap-close-tag",
      },
    ],
  });
}
...
if (bodyId === "page-user-edit") {
  // ページIDが「page-user-edit」の場合に処理を実行
  // 各科目の入力エリアを取得
  ...
  function AlertSubjectSettingFirst() {
    if (!isAlertSubjectSettingFirstShown) {
      // フラグがfalseの場合のみ実行
  ...
  // 【2科目セット購入】の場合
  if (checkBoughtMainSubject(["twosubjectpack"], true)) {
    // 2科目セットを購入している場合
  ...
  // 【3科目セット購入】の場合
  if (checkBoughtMainSubject(["threesubjectpack"], true)) {
    // 3科目セットを購入している場合
...
if (bodyId === "page-user-profile") {
  $(".alert-success").html(
    '変更が保存されました。科目レベルを設定した場合、<a href="https://lms.waomirai.com/my/">受講カレンダー</a>で確認ができます'
  );
...
$(document).on("click", ".triger-download-memosheet-modal", function (e) {
  createModal({
    ...
    buttons: [
      {
        text: "哲学（思考力）",
        url: memosheetPhilosophy,
        class: "btn-primary",
        blank: true,
      },
      {
        text: "科学リテラシー",
        url: memosheetScience,
        class: "btn-primary",
        blank: true,
      },
      {
        text: "経済（お金）",
        url: memosheetEconomy,
        class: "btn-primary",
        blank: true,
      },
    ],
